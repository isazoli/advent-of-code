object Day03 {
  ...
  val map = "^v^v^v^v^v"                          //> map  : String = ^v^v^v^v^v
  ...
  assert(11 == calcRoboSanta("^v^v^v^v^v").size)
    val input2 = scala.io.Source.fromInputStream(getClass().getResourceAsStream("day3-input.txt"))
                                                  //> input2  : scala.io.BufferedSource = non-empty iterator
  
  calcRoboSanta(input2.getLines().mkString).size  //> res0: Int = 2360
